...
export class ExerciseComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.exerciseObj.exerciseTypeId === 0) {
      ...
      this.http.post("https://localhost:7210/api/ExerciseType", this.exerciseObj)
        .subscribe(
          (res: any) => {
      ...
      this.http.put(`https://localhost:7210/api/ExerciseType/${this.exerciseObj.exerciseTypeId}`, this.exerciseObj)
        .subscribe(
          (res: any) => {
            if (res.exerciseTypeId >= 0) {
              alert("Exercise Record Updated!");
              // Update the exercise in the local list immediately
              const index = this.submittedExercises.findIndex(exercise => exercise.exerciseTypeId === this.exerciseObj.exerciseTypeId);
              if (index !== -1) {
  ...
  fetchExerciseLogs(): void {
    this.http.get<any[]>('https://localhost:7210/api/ExerciseType')
      .subscribe(
        (data) => {
  ...
  deleteExercise(exerciseTypeId: number): void {
    if (confirm("Are you sure you want to delete this log?")) {
      this.http.delete(`https://localhost:7210/api/ExerciseType/${exerciseTypeId}`)
        .subscribe(
          (res: any) => {
            if (res) {
              alert("Exercise Record Deleted!");
              // Remove the deleted exercise from the local list
              this.submittedExercises = this.submittedExercises.filter(exercise => exercise.exerciseTypeId !== exerciseTypeId);
              this.cd.detectChanges(); // Trigger change detection
            } else {
